...
describe("<Event /> component", () => {
  ...
  test("render event group", () => {
    expect(EventWrapper.find(".event_group")).toHaveLength(1);
  });
  ...
  test("props event object has correct properties", () => {
    expect(EventWrapper.props().event).toHaveProperty("name");
  });

  test("read event name correctly", () => {
    expect(EventWrapper.props().event.name).toBe(
      "Manual for Misery - Or: How not to javascript…"
    );
  });

  test("read event group correctly", () => {
    expect(EventWrapper.props().event.group.name).toBe("codecentric Berlin");
  });
});
